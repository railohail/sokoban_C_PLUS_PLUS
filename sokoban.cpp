...
int main()
{
    ...
    sokoban sokoban_test;
    sokoban_test.which(&height,&width,&target);// chooses the mission that we want to run
    temp_target = "map.txt"; 
    ...
    sokoban_test.generatemap(&target,&temp_target,height,width);
    sokoban_test.print(&target,height,width);// the initial map that the  user gets 
    // now it's time to let the target start to move 
    int input = 0;
    std::string output;
    int flag = 0;
    sokoban_test.controlmapgenerate(control_map,&player_x,&player_y,&target,height,width);
    ...
    while(1)
    {
        ...
        {    
            ...
            output = sokoban_test.sokoban_read_input(input,&flag);
            ...
            sokoban_test.moveornot(control_map,&player_x,&player_y,height,width,output);
            
            

            //this moves with every switch
            //now write a something to show the vector
            sokoban_test.showcurrent(control_map,height,width);
            win = sokoban_test.did_you_win(control_map,height,width);
            if(win == 1)
            {
                system("cls");
                std::cout << "You win!!!\n";
